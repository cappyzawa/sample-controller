...
import (
	...
	samplecontroller "github.com/cappyzawa/sample-controller/pkg/apis/samplecontroller/v1alpha1"
	"github.com/cappyzawa/sample-controller/pkg/client/clientset/versioned/fake"
...
var (
	...
	noResyncPreiodFunc = func() time.Duration { return 0 }
)
...
func (f *fixture) newController() (*Controller, informers.SharedInformerFactory, kubeinformers.SharedInformerFactory) {
	...
	i := informers.NewSharedInformerFactory(f.client, noResyncPreiodFunc())
	k8sI := kubeinformers.NewSharedInformerFactory(f.kubeclient, noResyncPreiodFunc())

	c := NewController(f.kubeclient, f.client, k8sI.Apps().V1().Deployments(), i.Samplecontroller().V1alpha1().Foos())

	c.foosSynced = alwaysReady
	c.deploymentsSynced = alwaysReady
	c.recorder = &record.FakeRecorder{}

	for _, fl := range f.fooLister {
...
func (f *fixture) runController(fooName string, startInformers bool, expectError bool) {
	...
	actions := fileterInformerActions(f.client.Actions())
	...
	k8sActions := fileterInformerActions(f.kubeclient.Actions())
...
func fileterInformerActions(actions []core.Action) []core.Action {
	ret := []core.Action{}
